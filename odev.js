...
function katilimSaati(dersSayisi, dersSuresi) {
  if (isNaN(parseInt(dersSayisi)) || isNaN(parseInt(dersSuresi))
      || (dersSayisi <0 || dersSuresi <0)) {
    throw new Error("hata, input formatı uygun değil!");
  }
  let toplam = parseInt(dersSayisi) * parseInt(dersSuresi);
  return console.log(toplam);
}
